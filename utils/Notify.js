...
module.exports.clicked = async (notificationId) => {
    ...
    if(existing) {
        model.notify.findByPk(notificationId)
            .then(notif => { notif.update({ status: 'CLICKED' }) });
    }
}

module.exports.sent = async (status, deviceId, appId, campaignId, type) => {
    let clicked = await model.notify.create({ status: status, deviceId: deviceId, applicationId: appId, campaignId: campaignId, type: type });
    return clicked;
}